...
float P0_move[3] = {80, -50, -50};
float P1_move[3] = {80, -30, -30};
float P2_move[3] = {80, 30, -30};
float P3_move[3] = {80, 50, -50};
...
void updateCBC(float angle) {
    ...
    float* moved_points[4] = {P0_move, P1_move, P2_move, P3_move};
    
    for(int i=0; i<4; i++) {
        float x_translated = points[i][0] - pivot_x;
        float y_translated = points[i][1] - pivot_y;
        
        moved_points[i][0] = (x_translated * cos_t - y_translated * sin_t) + pivot_x;
        moved_points[i][1] = (x_translated * sin_t + y_translated * cos_t) + pivot_y;
        moved_points[i][2] = points[i][2];
    }
}

void setup() {
  Serial.begin(115200);
  Serial.println("Testing Rotated CBC Points");
  pwm.begin();
  pwm.setPWMFreq(50);
  delay(10);
...
void doLeg(Leg &leg, float t, float &y) {
  ...
  if(leg.state) {
    float x = P0_move[0];
    float z = P0_move[2];
    float stepSize = (P0_move[1] - P3_move[1])/steps;
    
    kakiKanan(leg, x, y, z, servoCoxa, servoFemur, servoTibia);
    y += stepSize;
  } else { 
    float x, y_swing, z;
    bezier(t, P0_move, P1_move, P2_move, P3_move, x, y_swing, z);
...
void loop() {
  while(true) {
    ...
    y_R1 = R1.state ? P3_move[1] : P0_move[1];
    y_R2 = R2.state ? P3_move[1] : P0_move[1];
    y_R3 = R3.state ? P3_move[1] : P0_move[1];
