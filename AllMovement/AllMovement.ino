...
Leg R3 = {{12, 13, 14}, false, 63.0, -83.5, -20.0, 45.0, {{150, -210, -100}, {150, -190, -80}, {150, -130, -80}, {150, -110, -100}}, {{150, -210, -100}, {150, -190, -80}, {150, -130, -80}, {150, -110, -100}}, &pwm_rl};

const int L_rotating[4][3] = {{-100, -50, -80}, {-100, -30, -60}, {-100, 30, -60}, {-100, 50, -80}};
const int R_rotating[4][3] = {{100, -50, -80}, {100, -30, -60}, {100, 30, -60}, {100, 50, -80}};
...
void bodyKinematic(Leg &leg, float &x, float &y, float &z) {
  ...
  tempX = xr * cos(beta_rad) + zr * sin(beta_rad);
  float tempZ = -xr * sin(beta_rad) + zr * cos(beta_rad);
  xr = tempX;
  zr = tempZ;

  tempY = yr * cos(alpha_rad) - zr * sin(alpha_rad);
  tempZ = yr * sin(alpha_rad) + zr * cos(alpha_rad);
  yr = tempY;
  zr = tempZ;
...
void doLeg(Leg &leg, float t, float &y) {
    ...
    if(leg.state) {
      ...
      float x_local = x_global - (rotating ? 0 : leg.offset_x);
      float y_local = y - (rotating ? 0 : leg.offset_y);
      float z_local = z_global - (rotating ? 0 : leg.offset_z);
      ...
      float x_local = x_global - (rotating ? 0 : leg.offset_x);
      float y_local = y_swing_global - (rotating ? 0 : leg.offset_y);
      float z_local = z_global - (rotating ? 0 : leg.offset_z);
...
void loop() {
  ...
  if (rotating) {
    ...
    for (int i = 0; i < 4; i++) {
      for (int j = 0; j < 3; j++) {
          L1.P[i][j] = L_rotating[i][j];
          L2.P[i][j] = L_rotating[i][j];
          L3.P[i][j] = L_rotating[i][j];
          R1.P[i][j] = R_rotating[i][j];
          R2.P[i][j] = R_rotating[i][j];
          R3.P[i][j] = R_rotating[i][j];
      }
    }
    ...
    for (int i = 0; i < 4; i++) {
      for (int j = 0; j < 3; j++) {
          L1.P[i][j] = L1.P_ori[i][j];
          L2.P[i][j] = L2.P_ori[i][j];
          L3.P[i][j] = L3.P_ori[i][j];
          R1.P[i][j] = R1.P_ori[i][j];
          R2.P[i][j] = R2.P_ori[i][j];
          R3.P[i][j] = R3.P_ori[i][j];
      }
    }

  }

  if (forceLeft > 0.0 || forceRight > 0.0) {
    if (leftActive) {
      direction = (angleLeft > 270 && angleLeft < 90) ? 0.0 : 180.0; 
    }
  } else {
    return;
  }
