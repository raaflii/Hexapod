...
void bodyKinematic(Leg &leg, float &x, float &y, float &z) {
  ...
  float tempX = xr * cos(gamma1) - yr * sin(gamma1);
  float tempY = xr * sin(gamma1) + yr * cos(gamma1);
  xr = tempX;
  yr = tempY;
...
void doLeg(Leg &leg, float t, float &y) {
  ...
  if(leg.state) {
    ...
    kakiKiri(leg, x, y_swing, z, servoCoxa, servoFemur, servoTibia);
  }
...
void loop() {
  if (Serial.available()) {
    ...
    float servoCoxa, servoFemur, servoTibia;
    kakiKiri(L3, x, y, z, servoCoxa, servoFemur, servoTibia);
  
    pwm.setPWM(L3.ch[0], 0, angleToPulse(servoCoxa));
    pwm.setPWM(L3.ch[1], 0, angleToPulse(servoFemur));
    pwm.setPWM(L3.ch[2], 0, angleToPulse(servoTibia));
  }
}